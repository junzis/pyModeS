...
Reuses _callsign.decode_callsign which expects a 48-bit int. Payload
bits 8-55 are the bottom 48 bits of the 56-bit payload, so we mask
with `(1 << 48) - 1` before calling the decoder.

Validator rejects all-zero payload, wrong BDS ID, and any 6-bit slot
whose index is not a valid callsign character (A-Z, space, 0-9). The
...
def is_bds20(payload: int) -> bool:
    """Return True if `payload` is a plausible BDS 2,0 report."""
    if payload == 0:
        return False

    # BDS identifier must be 0x20 (payload bits 0-7).
    if (payload >> 48) & 0xFF != 0x20:
        return False
...
def decode_bds20(payload: int) -> dict[str, Any]:
    """Decode the callsign from a BDS 2,0 payload."""
    cs_bits = payload & ((1 << 48) - 1)
