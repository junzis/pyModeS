...
- Q = 0: 100-foot interval, Gillham gray code. Re-orders the 11 non-M
  non-Q bits into a gray-coded (500-ft, 100-ft) pair and converts to
  feet via `alt = n500 * 500 + n100 * 100 - 1300`.
...
def altcode_to_altitude(ac: int) -> int | None:
    ...
    if m_bit == 0 and q_bit == 0:
        # 100-foot interval, Gillham gray code.
        ...
        # Re-order into the Gillham gray string:
        #   graystr = D2 D4 A1 A2 A4 B1 B2 B4 C1 C2 C4  (11 bits)
        # Split: gc500 = first 8, gc100 = last 3.
        gc500 = (
            (d2 << 7)
            | (d4 << 6)
            | (a1 << 5)
...
def _gray2int(n: int) -> int:
    """Convert an unsigned Gillham gray code to a plain binary integer."""
    n ^= n >> 8
    n ^= n >> 4
    n ^= n >> 2
    n ^= n >> 1
