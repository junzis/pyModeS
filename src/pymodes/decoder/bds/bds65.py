...
def decode_bds65(payload: int) -> dict[str, Any]:
    """Decode a BDS 6,5 payload (ADS-B operational status).
    ...
    capability_class = (payload >> 32) & 0xFFFF  # bits 8-23
    operational_mode = (payload >> 16) & 0xFFFF  # bits 24-39
    version = (payload >> 13) & 0x7  # bits 40-42
    nic_supplement_a = (payload >> 12) & 0x1  # bit 43
    nac_p = (payload >> 8) & 0xF  # bits 44-47
    sil = (payload >> 4) & 0x3  # bits 50-51
    hrd = (payload >> 2) & 0x1  # bit 53
    ...
    if version == 2:
        result["sil_supplement"] = (payload >> 1) & 0x1
