...
class TestBds65Airborne:
    def test_synthetic_airborne_status(self):
        ...
        me = 0
        me |= tc << 51  # bits 0-4
        me |= subtype << 48  # bits 5-7
        # version at bits 40-42 → LSB pos 55-40=15 down to 55-42=13
        me |= version << 13
        me |= nic_s_a << 12  # bit 43 → LSB pos 12
        me |= nac_p << 8  # bits 44-47 → LSB pos 11..8
        me |= sil << 4  # bits 50-51 → LSB pos 5..4
        # nic_baro at bit 52 → LSB pos 3
        me |= nic_baro << 3

        result = decode_bds65(me)
    ...
    def test_dispatch_via_adsb_class(self):
        ...
        tc = 31
        me = tc << 51  # minimum viable TC=31 ME field
        # Full DF17: DF=17 (10001), CA=5, ICAO=400000, ME, CRC
        df_ca = (17 << 3) | 5  # 0x8D
        icao = 0x400000
        n = (df_ca << 104) | (icao << 80) | (me << 24)
...
class TestBds65Surface:
    ...
    def test_surface_no_nic_baro(self):
        ...
        me = (31 << 51) | (1 << 48) | (2 << 13)  # subtype=1, version=2
        result = decode_bds65(me)
    ...
    def test_airborne_v0_no_nic_baro(self):
        ...
        me = (31 << 51) | (0 << 48) | (0 << 13)  # subtype=0, version=0
        result = decode_bds65(me)
        assert "nic_baro" not in result
